...
project "Burnout3"
	...
	filter {}
	if(os.getenv("B3_RE_DIR")) then
		setpaths(os.getenv("B3_RE_DIR") .. "/", "%(cfg.buildtarget.name)")
	end
	...
	filter "platforms:*librw*"
		...
		includedirs { Librw }
		if(_OPTIONS["with-librw"]) then
			files { addSrcFiles("vendor/librw/skeleton"), addSrcFiles("vendor/librw/skeleton/imgui") }
			includedirs { "vendor/librw/skeleton", "vendor/librw/skeleton/imgui" }
